...
void GGEditor_ConnectionSlotTest::init()
{
    ...
    GGAbstractCommandFactory::oneShotCommand(m_cmdFac->createScene("default", "default"));
    ...
    GGAbstractCommandFactory::oneShotCommand(m_cmdFac->createConditionPage(m_scn));
    GGAbstractCommandFactory::oneShotCommand(m_cmdFac->createActionPage(m_scn));
    GGAbstractCommandFactory::oneShotCommand(m_cmdFac->createDecisionPage(m_scn));
    ...
    m_sc->verify("init", 1, 0, 5, 0, 0, 0, 0);
}
...
void GGEditor_ConnectionSlotTest::testConnectStart()
{
    ...
    QVERIFY2(m_s->startConnection() == cn, "Connection not set in StartPage");
    QVERIFY2(m_model->getPageIncommingConnections(m_e).value(0) == cn, "Connection not incomming for start page");
    VERIFYSIG(m_sc, "Create Connection", 0, 0, 0, 0, 1, 1, 2);
    ...
    VERIFYSIG(m_sc, "Exchange Connection", 0, 0, 0, 0, 1, 1, 2);
    ...
    VERIFYSIG(m_sc, "Undo Exchange Connection", 0, 0, 0, 0, 1, 1, 2);
...
void GGEditor_ConnectionSlotTest::testConnectCondition()
{
    ...
    VERIFYSIG(m_sc, "Connect Condition", 0, 0, 0, 0, 2, 0, 2);
}
...
void GGEditor_ConnectionSlotTest::testActionConnection()
{
    ...
    VERIFYSIG(m_sc, "Connect Action", 0, 0, 0, 0, 1, 0, 1);
}
...
void GGEditor_ConnectionSlotTest::testMappedConnection()
{
    ...
    mcp->addMappedLink(GGMappedLink::rectangle(QRect()));
    VERIFYSIG(m_sc, "Add Map Links", 0, 0, 0, 0, 0, 0, 2);
    ...
    VERIFYSIG(m_sc, "Connect Map", 0, 0, 0, 0, 1, 0, 1);
    GGConnection *c1 = static_cast<GGCreateConnectionCmd*> (m_stack->undoCommand())->createdConnection();
...
void GGEditor_ConnectionSlotTest::testDecisionConnection()
{
    ...
    m_d->addDecisionLink(GGLink());
    VERIFYSIG(m_sc, "Add Decisions", 0, 0, 0, 0, 0, 0, 2);
    ...
    VERIFYSIG(m_sc, "Connect Decision", 0, 0, 0, 0, 1, 0, 1);
    GGConnection *c1 = static_cast<GGCreateConnectionCmd*> (m_stack->undoCommand())->createdConnection();
