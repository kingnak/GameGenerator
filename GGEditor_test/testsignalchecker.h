...
public:
    ...
    void verify(QString s, int sr, int su, int pr, int pu, int cr, int cu, int pc, int vpc, bool r = true);
    bool verifyFL(QString s, QString f, int l, int sr, int su, int pr, int pu, int cr, int cu, int pc, int vpc, bool r = true);
    void verifyNull(QString s, bool r = true);
    void reset();
...
private:
    int m_sr, m_su, m_pr, m_pu, m_cr, m_cu, m_pc, m_vpc;
};

#define VERIFYVSIGR_STOP(sc, statement, SR, SU, PR, PU, CR, CU, PC, VPC, R) \
do {\
    if (!(sc)->verifyFL(statement, __FILE__, __LINE__, SR, SU, PR, PU, CR, CU, PC, VPC, R)) {\
        return;\
    }\
} while (0)

#define VERIFYVSIGR(sc, statement, SR, SU, PR, PU, CR, CU, PC, VPC, R) \
    (sc)->verifyFL(statement, __FILE__, __LINE__, SR, SU, PR, PU, CR, CU, PC, VPC, R)

#define VERIFYVSIG(sc, statement, SR, SU, PR, PU, CR, CU, PC, VPC) VERIFYVSIGR(sc, statement, SR, SU, PR, PU, CR, CU, PC, VPC, true)
#define VERIFYVSIG_STOP(sc, statement, SR, SU, PR, PU, CR, CU, PC, VPC) VERIFYVSIGR_STOP(sc, statement, SR, SU, PR, PU, CR, CU, PC, VPC, true)

#define VERIFYVSIGNULL(sc, statement) VERIFYVSIGR(sc, statement, 0, 0, 0, 0, 0, 0, 0, 0, true)
