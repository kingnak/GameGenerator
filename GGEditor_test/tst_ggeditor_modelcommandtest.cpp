...
void GGEditor_ModelCommandTest::init()
{
    ...
    m_s = m_model->factory()->createScene();
    m_model->registerNewScene(m_s);
...
{
    GGCreatePageCmd *cmd = m_fac->createStartPage(m_s);

    QVERIFY2(cmd->execute(), qPrintable("Error executing CreatePage command: " + cmd->error()));
    VERIFYSIG(m_sc, "Create Start Page Cmd", 0, 0, 1, 0, 0, 0, 0);
    ...
    VERIFYSIG(m_sc, "Undo Create Start Page", 0, 0, 0, 1, 0, 0, 0);
    ...
    VERIFYSIG(m_sc, "Redo Create Start Page Cmd", 0, 0, 1, 0, 0, 0, 0);
    ...
    VERIFYSIG(m_sc, "Second Undo Create Start Page", 0, 0, 0, 1, 0, 0, 0);
...
{
    GGCreatePageCmd *cr = m_fac->createStartPage(m_s);
    cr->execute();
    ...
    VERIFYSIG(m_sc, "Delete Page", 0, 0, 0, 1, 0 ,0, 0);
    ...
    VERIFYSIG(m_sc, "Undo Delete Page", 0, 0, 1, 0, 0 ,0, 0);
    ...
    VERIFYSIG(m_sc, "Redo Delete Page", 0, 0, 0, 1, 0 ,0, 0);
    ...
    VERIFYSIG(m_sc, "Second Undo Page", 0, 0, 1, 0, 0 ,0, 0);
...
{
    GGAbstractCommandFactory::oneShotCommand(m_fac->createStartPage(m_s));
    GGAbstractCommandFactory::oneShotCommand(m_fac->createEndPage(m_s));
    ...
    VERIFYSIG(m_sc, "Create Connection", 0, 0, 0, 0, 1, 0, 1);
    ...
    VERIFYSIG(m_sc, "Undo Create Connection", 0, 0, 0, 0, 0, 1, 1);
    ...
    VERIFYSIG(m_sc, "Redo Create Connection", 0, 0, 0, 0, 1, 0, 1);
    ...
    VERIFYSIG(m_sc, "Second Undo Create Connection", 0, 0, 0, 0, 0, 1, 1);
...
void GGEditor_ModelCommandTest::testDeleteConnection()
{
    ...
    VERIFYSIG(m_sc, "Delete Connectin", 0, 0, 0, 0, 0, 1, 1);
    ...
    VERIFYSIG(m_sc, "Undo Delete Connectin", 0, 0, 0, 0, 1, 0, 1);
    ...
    VERIFYSIG(m_sc, "Redo Delete Connectin", 0, 0, 0, 0, 0, 1, 1);
    ...
    VERIFYSIG(m_sc, "Second Undo Delete Connectin", 0, 0, 0, 0, 1, 0, 1);
...
void GGEditor_ModelCommandTest::testDeletePageWithConnection()
{
    ...
    VERIFYSIG(m_sc, "Delete Page with connnection", 0, 0, 0, 1, 0, 1, 0);
    ...
    VERIFYSIG(m_sc, "Undo Delete Page with connnection", 0, 0, 1, 0, 1, 0, 1);
    ...
    VERIFYSIG(m_sc, "Redo Delete Page with connnection", 0, 0, 0, 1, 0, 1, 0);
    ...
    VERIFYSIG(m_sc, "Second Undo Delete Page with connnection", 0, 0, 1, 0, 1, 0, 1);
    ...
    VERIFYSIG(m_sc, "Delete Page with connnection", 0, 0, 0, 1, 0, 1, 1);
    ...
    VERIFYSIG(m_sc, "Redo Delete Page with connnection", 0, 0, 0, 1, 0, 1, 1);
...
{
    GGAbstractCommandFactory::oneShotCommand(m_fac->createActionPage(m_s));
...
//    VERIFYSIGNULL(m_sc, "Setting same scene resulted in signal");
    ...
    VERIFYSIG(m_sc, "Set action link", 0, 0, 0, 0, 0, 0, 1);
    QCOMPARE(p->actionLink().name(), QString("link"));
    QVERIFY2(m_stk->undo(), "Cannot undo set page action link");
    VERIFYSIG(m_sc, "Undo Set action link", 0, 0, 0, 0, 0, 0, 1);
    QCOMPARE(p->actionLink().name(), QString());
    QVERIFY2(m_stk->redo(), "Cannot undo set page action link");
    VERIFYSIG(m_sc, "Redo Set action link", 0, 0, 0, 0, 0, 0, 1);
    QCOMPARE(p->actionLink().name(), QString("link"));

    m_stk->execute(m_fac->createEndPage(m_s));
    ...
    VERIFYSIG(m_sc, "Set action link with existing connection", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY2(p->actionLink().connection() == c, "Connection not set in new link");
    m_stk->undo();
    VERIFYSIG(m_sc, "Undo Set action link with existing connection", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY2(p->actionLink().connection() == c, "Connection not set in new link after undo");
    m_stk->redo();
    VERIFYSIG(m_sc, "Redo Set action link with existing connection", 0, 0, 0, 0, 0, 0, 1);
...
void GGEditor_ModelCommandTest::testMappedLinkCommands()
{
    ...
    VERIFYSIG(m_sc, "Add mapped link", 0, 0, 0, 0, 0, 0, 1);
    ...
    ml.setLink(l);
    QVERIFY2(!m_stk->execute(m_fac->setMappedLink(p, 0, ml)), "Can set mapped link with dummy connection");
    ...
    VERIFYSIG(m_sc, "Undo Add mapped link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY(p->getLinkMap().isEmpty());
    QVERIFY2(m_stk->redo(), "Cannot redo adding mapped link");
    VERIFYSIG(m_sc, "Redo Add mapped link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY(p->getLinkMap().size() == 1);
    ...
    VERIFYSIG(m_sc, "Create Mapped connection", 0, 0, 0, 0, 1, 0, 1);
    ...
    VERIFYSIG(m_sc, "Undo Create Mapped connection", 0, 0, 0, 0, 0, 1, 1);
    ...
    VERIFYSIG(m_sc, "Redo Create Mapped connection", 0, 0, 0, 0, 1, 0, 1);
    ...
    l.setName("a");
    ml.setLink(l);
    // Must have a connection so that this test makes sense
    QVERIFY(ml.link().connection() != NULL);
    QVERIFY2(m_stk->execute(m_fac->setMappedLink(p, 0, ml)), "Cannot update mapped link");
    VERIFYSIG(m_sc, "Update mapped link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY2(p->getLinkMap()[0].link().connection() != NULL, "No connection set in mapped link after update");
    QVERIFY2(m_stk->undo(), "Cannot undo update mapped link");
    VERIFYSIG(m_sc, "Undo update mapped link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY2(p->getLinkMap()[0].link().connection() != NULL, "No connection set in mapped link after undo update");
    QVERIFY2(m_stk->redo(), "Cannot redo update mapped link");
    VERIFYSIG(m_sc, "Redo update mapped link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY2(p->getLinkMap()[0].link().connection() != NULL, "No connection set in mapped link after redo update");


    // RESET
    QVERIFY2(m_stk->execute(m_fac->setMappedLink(p, 0, GGMappedLink::rectangle(QRect()))), "Cannot reset mapped link");
    VERIFYSIG(m_sc, "Reset mapped link", 0, 0, 0, 0, 0, 0, 1);
    ...
    VERIFYSIG(m_sc, "Undo Reset mapped link", 0, 0, 0, 0, 0, 0, 1);
    ...
    VERIFYSIG(m_sc, "Redo Reset mapped link", 0, 0, 0, 0, 0, 0, 1);
    ...
    VERIFYSIG(m_sc, "Remove mapped link", 0, 0, 0, 0, 0, 1, 1);
}

void GGEditor_ModelCommandTest::testDecisionLinkCommands()
{
    GGAbstractCommandFactory::oneShotCommand(m_fac->createDecisionPage(m_s));
    GGAbstractCommandFactory::oneShotCommand(m_fac->createEndPage(m_s));
    ...
    VERIFYSIG(m_sc, "Add decision link", 0, 0, 0, 0, 0, 0, 1);
    ...
    VERIFYSIG(m_sc, "Undo Add decision link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY(p->getDecisionLinks().isEmpty());
    QVERIFY2(m_stk->redo(), "Cannot redo adding decision link");
    VERIFYSIG(m_sc, "Redo Add decision link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY(p->getDecisionLinks().size() == 1);
    ...
    VERIFYSIG(m_sc, "Create decision connection", 0, 0, 0, 0, 1, 0, 1);
    ...
    VERIFYSIG(m_sc, "Undo Create decision connection", 0, 0, 0, 0, 0, 1, 1);
    ...
    VERIFYSIG(m_sc, "Redo Create decision connection", 0, 0, 0, 0, 1, 0, 1);
    QVERIFY2(p->getDecisionLinks()[0].connection() != NULL, "No connection set in decision link after redo");

    // UPDATE
    l = p->getDecisionLinks()[0];
    l.setName("a");
    // Must have a connection so that this test makes sense
    QVERIFY(l.connection() != NULL);
    QVERIFY2(m_stk->execute(m_fac->setDecisionLink(p, 0, l)), "Cannot update decision link");
    VERIFYSIG(m_sc, "Update decision link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY2(p->getDecisionLinks()[0].connection() != NULL, "No connection set in decision link after update");
    QVERIFY2(m_stk->undo(), "Cannot undo update decision link");
    VERIFYSIG(m_sc, "Undo update decision link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY2(p->getDecisionLinks()[0].connection() != NULL, "No connection set in decision link after undo update");
    QVERIFY2(m_stk->redo(), "Cannot redo update decision link");
    VERIFYSIG(m_sc, "Redo update decision link", 0, 0, 0, 0, 0, 0, 1);
    QVERIFY2(p->getDecisionLinks()[0].connection() != NULL, "No connection set in decision link after redo update");

    // RESET
    QVERIFY2(m_stk->execute(m_fac->setDecisionLink(p, 0, GGLink())), "Cannot reset decision link");
    VERIFYSIG(m_sc, "Reset decision link", 0, 0, 0, 0, 0, 0, 1);
    ...
    VERIFYSIG(m_sc, "Undo Reset decision link", 0, 0, 0, 0, 0, 0, 1);
    ...
    VERIFYSIG(m_sc, "Redo Reset decision link", 0, 0, 0, 0, 0, 0, 1);
    ...
    VERIFYSIG(m_sc, "Remove decision link", 0, 0, 0, 0, 0, 1, 1);
}
